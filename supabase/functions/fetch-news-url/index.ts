...
Deno.serve(withPerf({ functionName: 'fetch-news-url' }, async (req) => {
  ...
  try {
    ...
    console.log('Fetching news from URL:', formattedUrl);
    ...
    const summary = data.data?.summary || data.summary || data.data?.metadata?.description || data.metadata?.description || '';

    console.log('Fetched news successfully:', { title: title.substring(0, 50) });
