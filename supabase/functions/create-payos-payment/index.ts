...
Deno.serve(withPerf({ functionName: 'create-payos-payment' }, async (req) => {
  ...
  try {
    ...
    if (voucher_code && typeof voucher_code === 'string' && voucher_code.trim()) {
      ...
      amount = Math.max(2000, amount - discountAmount); // payOS minimum 2000₫
      console.log(`Voucher ${code}: discount ${discountAmount}₫, final amount ${amount}₫`);
    }

    // Create unique order code (payOS requires positive integer)
    ...
    const checksum = await hmacSHA256(payosChecksumKey, checksumData);
    ...
    const payosResponse = await fetch('https://api-merchant.payos.vn/v2/payment-requests', {
      ...
      body: JSON.stringify({
        ...
        description,
        items: [
          { name: `Goi ${plan_type} (${cycle})`, quantity: 1, price: amount }
        ],
        cancelUrl,
    ...
    console.log('payOS response:', JSON.stringify(payosResult));
    ...
    return new Response(JSON.stringify({
      payment_url: payosResult.data.checkoutUrl,
