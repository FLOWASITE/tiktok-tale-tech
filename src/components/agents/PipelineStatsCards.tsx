...
export function PipelineStatsCards({ pipelines }: PipelineStatsCardsProps) {
  ...
  const stats = [
    { label: 'Hôm nay', value: todayPipelines.length, icon: Clock, color: 'text-blue-500' },
    { label: 'Tỷ lệ thành công', value: `${successRate}%`, icon: CheckCircle2, color: 'text-emerald-500' },
    { label: 'TB hoàn thành', value: avgTimeMin > 0 ? `${avgTimeMin} phút` : '—', icon: Activity, color: 'text-primary' },
    { label: 'Đang chạy', value: running.length, icon: Activity, color: 'text-amber-500', pulse: running.length > 0 },
  ];
