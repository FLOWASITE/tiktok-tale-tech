          ...
          {!isGenerating && step === confirmStep && (() => {
            ...
            return (
              <div className="space-y-2.5">
                ...
                {/* Lịch nội dung chi tiết theo ngày */}
                {estimatedPosts > 0 && selectedChannels.length > 0 && (() => {
                  // Build deterministic schedule
                  type Slot = { date: Date; channelId: string; pillar: string; topic: string };
                  const DOW_LABELS = ['CN', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7'];
                  // Channel → preferred weekdays by frequency (0=Sun..6=Sat)
                  ...
                  // Deterministic shuffle seeded by campaign name
                  const seedStr = (name || description || 'flowa') + selectedChannels.join(',');
                  let seed = 0;
                  ...
                  // Pillar pool (weighted) — fallback "Mixed"
                  const pillarEntries = Object.entries(pillarAllocation).filter(([, p]) => p > 0);
                  const pillarPool: string[] = pillarEntries.length > 0
                    ? pillarEntries.flatMap(([p, pct]) => Array(Math.max(1, Math.round(pct / 5))).fill(p))
                    : ['Mixed'];
                  // shuffle
                  for (let i = pillarPool.length - 1; i > 0; i--) {
                    const j = Math.floor(rand() * (i + 1));
                    [pillarPool[i], pillarPool[j]] = [pillarPool[j], pillarPool[i]];
                  }

                  // Topic templates per pillar
                  const topicHints: string[] = keyMessages.length > 0 ? keyMessages : [];
                  const channelTopicIdx: Record<string, number> = {};
                  const buildTopic = (pillar: string, channelLabel: string) => {
                    const idx = (channelTopicIdx[channelLabel] = (channelTopicIdx[channelLabel] || 0) + 1);
                    if (topicHints.length > 0) {
                      const hint = topicHints[(idx - 1) % topicHints.length];
                      return `${hint}`;
                    }
                    const verbs: Record<string, string> = {
                      educate: 'Hướng dẫn', inspire: 'Câu chuyện', sell: 'Ưu đãi',
                      entertain: 'Khoảnh khắc', mixed: 'Chủ đề',
                    };
                    const verb = verbs[pillar.toLowerCase()] || pillar;
                    return `${verb} #${idx} · ${channelLabel}`;
                  };
                  ...
                  selectedChannels.forEach(ch => {
                    const info = AVAILABLE_CHANNELS.find(c => c.id === ch);
                    const label = info?.label || ch;
                    const f = frequency[ch] || 'weekly';
                    const allowedDays = freqDays[f] || [3];
                    for (let day = 0; day < effectiveDuration; day++) {
                      const d = new Date(start.getTime() + day * 86400000);
                      if (allowedDays.includes(d.getDay())) {
                        const pillar = pillarPool[pillarCursor++ % pillarPool.length];
                        slots.push({ date: d, channelId: ch, pillar, topic: buildTopic(pillar, label) });
                      }
                    }
                  });
                  ...
                  // Group by week
                  type WeekGroup = { weekIdx: number; rangeLabel: string; slots: Slot[] };
                  const weekGroups: WeekGroup[] = [];
                  slots.forEach(s => {
                    const diffDays = Math.floor((s.date.getTime() - start.getTime()) / 86400000);
                    const wIdx = Math.floor(diffDays / 7);
                    let g = weekGroups.find(x => x.weekIdx === wIdx);
                    if (!g) {
                      const wStart = new Date(start.getTime() + wIdx * 7 * 86400000);
                      const wEnd = new Date(wStart.getTime() + 6 * 86400000);
                      const rangeLabel = `${wStart.getDate()}/${wStart.getMonth() + 1} – ${wEnd.getDate()}/${wEnd.getMonth() + 1}`;
                      g = { weekIdx: wIdx, rangeLabel, slots: [] };
                      weekGroups.push(g);
                    }
                    g.slots.push(s);
                  });
                  ...
                  const pillarClass = (p: string) => pillarBadgeClass[p.toLowerCase()] || 'bg-muted text-muted-foreground border-border';

                  const renderRow = (s: Slot, key: React.Key) => {
                    const info = AVAILABLE_CHANNELS.find(c => c.id === s.channelId);
                    return (
                      <div key={key} className="flex items-center gap-2 px-2 py-1 rounded hover:bg-muted/40 transition-colors">
                        <span className="w-14 text-[10px] text-muted-foreground tabular-nums shrink-0">
                          {DOW_LABELS[s.date.getDay()]} {s.date.getDate()}/{s.date.getMonth() + 1}
                        </span>
                        <div className="flex items-center gap-1 w-24 shrink-0">
                          <ChannelIcon channel={info?.channelKey || 'website'} size={11} className={channelIconColors[info?.channelKey || 'website']} />
                          <span className="text-[10px] font-medium truncate">{info?.label || s.channelId}</span>
                        </div>
                        <Badge variant="outline" className={cn('text-[9px] h-4 px-1.5 font-normal shrink-0 capitalize', pillarClass(s.pillar))}>
                          {s.pillar}
                        </Badge>
                        <span className="flex-1 text-[11px] text-foreground/90 truncate min-w-0">{s.topic}</span>
                      </div>
                    );
                  };

                  const renderGroups = (groups: WeekGroup[]) => groups.map(g => (
                    <div key={g.weekIdx} className="space-y-0.5">
                      <div className="flex items-center justify-between px-2 py-1 bg-muted/40 rounded sticky top-0 backdrop-blur-sm">
                        <span className="text-[10px] font-semibold">Tuần {g.weekIdx + 1} <span className="font-normal text-muted-foreground">({g.rangeLabel})</span></span>
                        <span className="text-[9px] text-muted-foreground tabular-nums">{g.slots.length} bài</span>
                      </div>
                      <div className="space-y-0">
                        {g.slots.map((s, i) => renderRow(s, `${g.weekIdx}-${i}`))}
                      </div>
                    </div>
                  ));

                  const previewGroups = weekGroups.slice(0, 4);
                  const hasMore = weekGroups.length > previewGroups.length;
                  const defaultOpen = slots.length <= 14;

                  return (
                    <Collapsible defaultOpen={defaultOpen}>
                      <div className="rounded-lg border bg-card overflow-hidden">
                        <CollapsibleTrigger className="flex items-center justify-between w-full px-2.5 py-2 hover:bg-muted/40 transition-colors group">
                          <div className="flex items-center gap-1.5">
                            <CalendarDays className="w-3 h-3 text-primary" />
                            <span className="text-[11px] font-medium">Lịch nội dung chi tiết</span>
                            <Badge variant="secondary" className="text-[9px] h-4 px-1.5 font-normal">
                              {slots.length} bài · {totalWeeks} tuần
                            </Badge>
                          </div>
                        ...
                        <CollapsibleContent>
                          <div className="border-t">
                            <div className="max-h-[280px] overflow-y-auto p-1.5 space-y-2">
                              {renderGroups(previewGroups)}
                            </div>
                            {hasMore && (
                              <Dialog>
                                <DialogTrigger asChild>
                                  <button className="w-full px-2.5 py-1.5 text-[10px] text-primary hover:bg-primary/5 border-t font-medium flex items-center justify-center gap-1">
                                    Xem tất cả {slots.length} bài ({weekGroups.length} tuần)
                                ...
                                <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
                                  <DialogHeader>
                                    <DialogTitle className="flex items-center gap-2 text-sm">
                                      ...
                                      Lịch nội dung đầy đủ
                                      <Badge variant="secondary" className="text-[10px] font-normal">{slots.length} bài · {totalWeeks} tuần</Badge>
                                    </DialogTitle>
                                  </DialogHeader>
                            ...
                            {campaignStartDate === new Date().toISOString().split('T')[0] && (
                              <p className="px-2.5 py-1 text-[9px] text-muted-foreground italic border-t bg-muted/20">
                                * Lịch tính từ hôm nay. Có thể điều chỉnh ngày bắt đầu ở bước trước.
                              </p>
                            )}
                          </div>
                        </CollapsibleContent>
