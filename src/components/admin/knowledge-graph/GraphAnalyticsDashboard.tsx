...
function HealthChecksPanel({ embeddingCoverage, totalNodes, totalEdges, nodesByType }: HealthChecksPanelProps) {
  const checks: HealthCheck[] = [
    {
      name: "Embedding Coverage",
      status: embeddingCoverage >= 90 ? 'pass' : embeddingCoverage >= 50 ? 'warn' : 'fail',
      message: `${embeddingCoverage.toFixed(1)}% of nodes have embeddings`,
      action: embeddingCoverage < 90 ? "Generate embeddings for remaining nodes" : undefined,
    },
    {
      name: "Graph Connectivity",
      status: totalEdges >= totalNodes * 0.5 ? 'pass' : totalEdges >= totalNodes * 0.2 ? 'warn' : 'fail',
      message: `${(totalEdges / Math.max(totalNodes, 1)).toFixed(2)} edges per node`,
      action: totalEdges < totalNodes * 0.5 ? "Create more connections between nodes" : undefined,
    },
    {
      name: "Regulation Coverage",
      status: nodesByType.regulation >= 10 ? 'pass' : nodesByType.regulation >= 1 ? 'warn' : 'fail',
      message: `${nodesByType.regulation || 0} regulation nodes`,
      action: (nodesByType.regulation || 0) < 10 ? "Extract regulations from industry packs" : undefined,
    },
    {
      name: "Term Coverage",
      status: nodesByType.term >= 50 ? 'pass' : nodesByType.term >= 10 ? 'warn' : 'fail',
      message: `${nodesByType.term || 0} term nodes`,
      action: (nodesByType.term || 0) < 50 ? "Extract terms from industry packs" : undefined,
    },
    {
      name: "Graph Size",
      status: totalNodes >= 100 ? 'pass' : totalNodes >= 10 ? 'warn' : 'fail',
      message: `${totalNodes} total nodes`,
      action: totalNodes < 100 ? "Add more nodes to enrich the graph" : undefined,
    },
  ];

  const passCount = checks.filter(c => c.status === 'pass').length;
  const overallHealth = passCount / checks.length * 100;
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        {checks.map((check) => (
          <div key={check.name} className="flex items-start gap-3 p-2 rounded-lg bg-muted/50">
            {check.status === 'pass' ? (
