                  ...
                  return (
                    <Tooltip key={idx}>
                      <TooltipTrigger asChild>
                        <div
                          className={cn(
                            "group relative flex w-full items-start gap-2 px-3 py-2 pr-12 rounded-xl border cursor-pointer transition-all text-xs xs:text-sm",
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <span
                              className="block text-xs xs:text-sm leading-snug line-clamp-2 font-medium"
                            ...
                            {/* Meta badges row */}
                            {showEnhancedInfo && (score !== undefined || suggestion.estimatedEngagement === 'high' || suggestion.relatedEvent) && (
                              <div className="flex items-center gap-1 mt-1 flex-wrap">
                                {score !== undefined && (
                                  <span className={cn(
                                    "text-[9px] xs:text-[10px] font-semibold px-1.5 py-0.5 rounded-full",
                                    getScoreColor(score)
                                  )}>
                                    {score}
                                  </span>
                                ...
                                {suggestion.estimatedEngagement === 'high' && (
                                  <span className={cn(
                                    "text-[8px] xs:text-[9px] font-bold px-1 py-0.5 rounded uppercase",
                                ...
                                {suggestion.relatedEvent && (
                                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-600 truncate max-w-[200px]">
                                    📅 {suggestion.relatedEvent}
                                  </span>
                                )}
                              </div>
                          ...
                          {/* Action Buttons - Absolute top-right, show on hover */}
                          {(onSave || onFeedback) && !disabled && (
                            <div className="absolute top-1.5 right-1.5 hidden group-hover:flex items-center gap-0.5 bg-background/95 backdrop-blur rounded-md px-1 py-0.5 border border-border/50 shadow-sm">
                              {onSave && !isSaved && (
                                <button
                                  type="button"
