...
import { Eye, Clock, Target, Layers, Megaphone, FileText } from 'lucide-react';
...
export const AdCopyCard = memo(function AdCopyCard({ 
  ...
  const handleStatusChange = useCallback((status: string) => {
    onStatusChange?.(adCopy.id, status);
  }, [adCopy.id, onStatusChange]);
  const handleQuickPreview = useCallback(() => setShowQuickPreview(true), []);
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-4">
              ...
              <div className="hidden lg:block">
                <StatusTimeline currentStatus={adCopy.status} compact />
              </div>
    ...
    <>
      <Card className="group relative hover:shadow-xl hover:shadow-primary/5 hover:border-primary/30 transition-all duration-300 overflow-hidden bg-background/60 backdrop-blur-sm">
        <CardContent className="p-0">
          ...
          {/* Actions - Simplified */}
          <div className="px-4 py-3 border-t border-border/50 bg-gradient-to-r from-muted/30 to-muted/10">
            <Button 
              variant="ghost" 
              size="sm" 
              onClick={handleView} 
