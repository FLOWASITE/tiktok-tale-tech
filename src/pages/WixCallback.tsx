...
export default function WixCallback() {
  ...
  const [message, setMessage] = useState('Đang xử lý kết nối Wix...');
  ...
  useEffect(() => {
    ...
    if (s === 'success') {
      setStatus('success');
      setMessage(site ? `Đã kết nối Wix: ${site}` : 'Kết nối Wix thành công!');
      const t = setTimeout(() => {
        if (window.opener) {
          window.close();
        } else {
    ...
    if (s === 'error') {
      setStatus('error');
      setMessage(errMsg ? decodeURIComponent(errMsg) : 'Kết nối Wix thất bại');
      return;
    }

    setStatus('error');
    setMessage('Không nhận được phản hồi từ Wix');
