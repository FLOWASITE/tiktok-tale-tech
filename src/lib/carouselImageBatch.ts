...
import type { Carousel, CarouselSlide } from '@/types/carousel';
...
export async function extractBrandColorsWithFallback(
  ...
  if (carousel.brand_guideline) {
    try {
      const parsed =
        typeof carousel.brand_guideline === 'string'
      ...
      if (parsed?.colors || parsed?.textColor) {
        return {
          ...
          backgroundColor:
            parsed.backgroundColor || parsed.colors?.background || parsed.colors?.secondary,
        };
      }
    } catch {
      const hex = (carousel.brand_guideline as string).match(/#[0-9A-Fa-f]{3,8}/g);
...
/**
 * Idempotent fire-and-forget launch of generate-carousel-images-batch.
 * Creates a generation_tasks row (unless one already exists for this carousel)
 * then invokes the edge function. Safe to call from anywhere — does not depend
 * on any UI component being mounted.
 */
export async function launchCarouselImageBatch({
  ...
  const { data: task, error } = await (supabase.from('generation_tasks') as any)
    .insert({
      user_id: userId,
  ...
  if (error || !task) {
    console.error('[launchCarouselImageBatch] create task failed:', error);
    return { taskId: null, alreadyRunning: false };
  }
  ...
  try {
    ...
    fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate-carousel-images-batch`, {
      ...
      body: JSON.stringify({ ...inputParams, taskId: task.id }),
    }).catch((err) => console.warn('[launchCarouselImageBatch] fire-and-forget error:', err));
  } catch (err) {
    console.warn('[launchCarouselImageBatch] invoke failed:', err);
  }
